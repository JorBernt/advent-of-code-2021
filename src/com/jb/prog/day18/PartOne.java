...
class PartOne {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Queue<SnailFish> fishes = new LinkedList<>();
        while (in.hasNextLine()) {
            fishes.add(parseNumber(new SnailFish(), in.nextLine()));
        }
        SnailFish root = fishes.poll();
        print(root);
        System.out.println();
        while (!fishes.isEmpty()) {
            root = add(root, fishes.poll());
            reduce(root);
        }
        print(root);
        System.out.println();
